...
if (isset($_POST['filter'])) {
    ...
    if ($role == 'admin') {
        $i = mysqli_query($conn, "SELECT expense.*, users.username, category.category_name FROM expense 
              ...
              WHERE expense.category_id = '$category_id' AND expense.family_id='$family_id'");
    } elseif ($role == 'user') {
        $i = mysqli_query($conn, "SELECT expense.*, users.username, category.category_name FROM expense 
              JOIN users ON expense.user_id = users.user_id 
              JOIN category ON expense.category_id = category.category_id 
              WHERE expense.category_id = '$category_id' AND expense.user_id='$user_id'");
    } else {
...
<body>
    <div class="container mt-5">
        <div class="card" style="width: 700px; margin: 50px auto; ">
            ...
            <div class="card-body">
                <p>Please enter the category ID to filter expenses.</p>
                ...
                <div class="mt-3">
                    <p>After filtering, you can view the expenses related to the selected category.</p>
                    <p class="mt-3"><strong>Note:</strong> Ensure that the Category ID is valid and exists in the database.</p>
                </div>
                <div>
